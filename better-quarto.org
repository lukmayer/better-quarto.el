...
* Workflow
- Insert code blocks with C-c-i
- Navigate up and down code-chunks with C-c-m and SPC-n (Only works when cursor is not inside code-chunks)
- Run code blocks in REPL via vterm with C-c-r (caveat: have to be on the ```lang line for this to work)
- Run above/below code blocks with C-c-a and C-c-b 
- Update styling with C-c-u
- Render document with C-c-k
- Insert HTML style block with C-c-s, HTML script blocks with C-c-j
- Insert quarto divs with C-c-q


* TODO
- Allow white space in YAML block detection
- highlight latex equations
- fix vterm buffer naming
- preview latex with math-preview
- include vterm and quarto-mode as dependencies
